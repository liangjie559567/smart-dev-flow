...
- Cancel with `/smart-dev-flow:cancel` at any time; progress is preserved for resume
</Execution_Policy>
...
6. **Phase 5 - Cleanup**: Delete all state files on successful completion
   ...
   - Run `/smart-dev-flow:cancel` for clean exit
</Steps>
...
If autopilot was cancelled or failed, run `/smart-dev-flow:autopilot` again to resume from where it stopped.
