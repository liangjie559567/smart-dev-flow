...
| Command | Action |
|---------|--------|
| `/smart-dev-flow:writer-memory init <project-name>` | Initialize new project memory |
| `/smart-dev-flow:writer-memory status` | Show memory overview (character count, scene count, etc) |
| `/smart-dev-flow:writer-memory char add <name>` | Add new character |
| `/smart-dev-flow:writer-memory char <name>` | View character details |
| `/smart-dev-flow:writer-memory char update <name> <field> <value>` | Update character field |
| `/smart-dev-flow:writer-memory char list` | List all characters |
| `/smart-dev-flow:writer-memory rel add <char1> <char2> <type>` | Add relationship |
| `/smart-dev-flow:writer-memory rel <char1> <char2>` | View relationship |
| `/smart-dev-flow:writer-memory rel update <char1> <char2> <event>` | Add relationship event |
| `/smart-dev-flow:writer-memory scene add <title>` | Add new scene |
| `/smart-dev-flow:writer-memory scene <id>` | View scene details |
| `/smart-dev-flow:writer-memory scene list` | List all scenes |
| `/smart-dev-flow:writer-memory theme add <name>` | Add theme |
| `/smart-dev-flow:writer-memory world set <field> <value>` | Set world attribute |
| `/smart-dev-flow:writer-memory query <question>` | Query memory naturally (Korean supported) |
| `/smart-dev-flow:writer-memory validate <character> <dialogue>` | Check if dialogue matches character tone |
| `/smart-dev-flow:writer-memory synopsis` | Generate emotion-focused synopsis |
| `/smart-dev-flow:writer-memory export` | Export full memory as readable markdown |
| `/smart-dev-flow:writer-memory backup` | Create manual backup |
...
```
Task(subagent_type="smart-dev-flow:architect",
     model="opus",
