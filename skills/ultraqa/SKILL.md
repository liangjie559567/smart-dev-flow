...
|------------|-----------|---------------|
| `/smart-dev-flow:ultraqa --tests` | tests | All test suites pass |
| `/smart-dev-flow:ultraqa --build` | build | Build succeeds with exit 0 |
| `/smart-dev-flow:ultraqa --lint` | lint | No lint errors |
| `/smart-dev-flow:ultraqa --typecheck` | typecheck | No TypeScript errors |
| `/smart-dev-flow:ultraqa --custom "pattern"` | custom | Custom success pattern in output |
...
1. **RUN QA**: Execute verification based on goal type
   ...
   - `--interactive`: Use qa-tester for interactive CLI/service testing:
     ```
     Task(subagent_type="smart-dev-flow:qa-tester", model="sonnet", prompt="TEST:
...
3. **ARCHITECT DIAGNOSIS**: Spawn architect to analyze failure
   ```
   Task(subagent_type="smart-dev-flow:architect", model="opus", prompt="DIAGNOSE FAILURE:
   Goal: [goal type]
...
4. **FIX ISSUES**: Apply architect's recommendations
   ```
   Task(subagent_type="smart-dev-flow:executor", model="sonnet", prompt="FIX:
...
User can cancel with `/smart-dev-flow:cancel` which clears the state file.
