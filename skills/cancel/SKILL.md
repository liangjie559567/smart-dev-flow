...
`/smart-dev-flow:cancel` follows the session-aware state contract:
...
```
/smart-dev-flow:cancel --force
```

```
/smart-dev-flow:cancel --all
```
...
3. If a `session_id` was supplied to `/smart-dev-flow:cancel`, skip legacy fallback entirely and operate solely within that session path; otherwise, consult legacy files in `.omc/state/*.json` only if the state tools report no active session. Swarm remains a shared SQLite/marker mode outside session scoping.
...
if [[ -f .omc/state/autopilot-state.json ]]; then
  ...
  echo "Run /smart-dev-flow:autopilot to resume."
fi
```
...
if [[ -f .omc/state/ultrawork-state.json ]]; then
  ...
  if [[ "$LINKED" == "true" ]]; then
    echo "Ultrawork is linked to Ralph. Use /smart-dev-flow:cancel to cancel both."
    exit 1
  fi
...
|------|-----------------|----------------|
| Autopilot | Yes (phase, files, spec, plan, verdicts) | `/smart-dev-flow:autopilot` |
| Ralph | No | N/A |
| Ultrawork | No | N/A |
| UltraQA | No | N/A |
| Swarm | No | N/A |
