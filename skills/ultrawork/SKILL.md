...
<Tool_Usage>
- Use `Task(subagent_type="smart-dev-flow:executor-low", model="haiku", ...)` for simple changes
- Use `Task(subagent_type="smart-dev-flow:executor", model="sonnet", ...)` for standard work
- Use `Task(subagent_type="smart-dev-flow:executor-high", model="opus", ...)` for complex work
...
```
Task(subagent_type="smart-dev-flow:executor-low", model="haiku", prompt="Add missing type export for Config interface")
Task(subagent_type="smart-dev-flow:executor", model="sonnet", prompt="Implement the /api/users endpoint with validation")
Task(subagent_type="smart-dev-flow:executor", model="sonnet", prompt="Add integration tests for the auth middleware")
```
...
```
Task(subagent_type="smart-dev-flow:executor", model="sonnet", prompt="npm install && npm run build", run_in_background=true)
Task(subagent_type="smart-dev-flow:executor-low", model="haiku", prompt="Update the README with new API endpoints")
```
...
```
Task(subagent_type="smart-dev-flow:executor-high", model="opus", prompt="Add a missing semicolon")
```
