...
try {
  learnFromToolOutput = (await import('../omc-dist/hooks/project-memory/learner.js')).learnFromToolOutput;
} catch (err) {
...
try {
  findProjectRoot = (await import('../omc-dist/hooks/rules-injector/finder.js')).findProjectRoot;
} catch (err) {
  if (err?.code === 'ERR_MODULE_NOT_FOUND' && /dist\//.test(err?.message)) {
